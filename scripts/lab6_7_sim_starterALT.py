...
class ObstacleAvoidingWaypointController:
    ...
    def obstacle_avoiding_control(self, visualize: bool = True):
        ...
        if self.distancel < self.distancer:
            ctrl_msg.angular.z = uang
        else:
            ctrl_msg.angular.z = -uang
        ######### Your code ends here #########
    ...
    def control_robot(self):
        ...
        while not rospy.is_shutdown():
            ...
            if not distances:
                ...
                distanceleft = min(left) if left else float("inf")
                distanceright = min(right) if right else float("inf")
            self.distancel = distanceleft
            self.distancer = distanceright
            self.distance = min(distanceleft, distanceright)
            if self.distancel < self.wall_following_desired_distance or self.distancer < self.wall_following_desired_distance:
